...
class HTMLToExcelApp:
    def __init__(self, root):
        self.root = root
        self.root.title("HTML ➜ Excel Converter - Ultimate GUI")
    ...
    def setup_ui(self):
        ...
        self.select_all_btn = ttk.Button(self.frame, text="✅ Chọn tất cả", command=self.select_all)
        self.select_all_btn.grid(row=4, column=0, pady=5)

        self.deselect_all_btn = ttk.Button(self.frame, text="❌ Bỏ chọn tất cả", command=self.deselect_all)
        self.deselect_all_btn.grid(row=4, column=1, pady=5)

        ttk.Button(self.frame, text="💾 Chọn nơi lưu Excel", command=self.choose_excel_path).grid(row=5, column=0, columnspan=3)

        entry_excel = ttk.Entry(self.frame, textvariable=self.excel_path, width=80)
        entry_excel.grid(row=6, column=0, columnspan=3, pady=5)
    ...
    def load_tables(self):
        ...
        try:
            ...
            self.tables = pd.read_html(str(soup))
            ...
            self.status_label.config(text=f"🔍 Đã phát hiện {len(self.tables)} bảng.", foreground="green")
